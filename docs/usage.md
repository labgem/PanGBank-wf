...
```bash
nextflow run labgem/pangbank --genomes input_genomes.tsv  --taxonomy genome_taxonomy.tsv --outdir ./results -profile docker
```
...
```bash
nextflow run labgem/pangbank -profile docker -params-file params.yaml
```
...
First, go to the [labgem/pangbank releases page](https://github.com/labgem/pangbank/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
