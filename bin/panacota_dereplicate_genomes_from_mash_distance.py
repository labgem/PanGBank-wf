...
def remove_close_genomes(
    genomes_under_selection,
    genome_to_index,
    mash_dist_matrix,
    genomes_removed,
    identity_cutoff,
...
def parse_mash_dist_result_into_matrix(
    genome_to_index: Dict[str, int], mash_result_file: Path, disable_bar: bool
):
    """ """

    if not mash_result_file.is_file():
        print(
            f"Matrix file {mash_result_file} does not exist. We cannot read it "
...
def select_genomes_like_panacota(
    mash_dist_matrix, sorted_genomes, genome_to_index, identity_cutoff, disable_bar
):
    ...
    with tqdm(total=genome_count, unit="genome", disable=disable_bar) as progress:
        ...
        while len(genomes_under_selection) > 1:
            ...
            genome_process_in_loop = (
                current_genome_process_count - last_genome_process_count
            )
            last_genome_process_count = current_genome_process_count
            progress.update(genome_process_in_loop)

    logging.info(
        "Final number of genomes in dataset: {}".format(
            genome_count - len(genomes_removed)
        )
    )

    selected_genomes = [
        genome for genome in sorted_genomes if genome not in genomes_removed
    ]

    return selected_genomes


def select_genome_with_hierarchical_clustering(
    mash_dist_matrix, num_clusters, index_to_genome, method
):
        ...
        for i in range(num_genomes):
            if i not in selected_indices:
                ...
                min_dist_to_selected = min(
                    [dist_matrix[i, j] for j in selected_indices]
                )
...
def main(argv=None):
    ...
    if sparse_matrix_file.is_file():
        ...
        sparse_similarity_matrix = parse_mash_dist_result_into_matrix(
    ...
    for distance_cutoff in [
        ...
        selected_genomes_panacota = select_genomes_like_panacota(
        ...
        write_selected_genomes_ids(
            selected_genomes_panacota, outfile=selected_genome_outfile
        )
